...
func TestClaudeClient_DefaultEndpoint(t *testing.T) {
	...
	if client.config.BaseURL != DefaultClaudeEndpoint {
		t.Errorf("Expected BaseURL %q, got %q", DefaultClaudeEndpoint, client.config.BaseURL)
	}
}

// TestClaudeClient_EndpointOverride tests that CLAUDE_API_ENDPOINT env var overrides the
// default endpoint.
func TestClaudeClient_EndpointOverride(t *testing.T) {
	...
	cfg := LLMConfig{
		Provider:  "claude",
		APIKeyEnv: "TEST_LLM_API_KEY_EP2",
	}
	...
	if client.config.BaseURL != customEndpoint {
		t.Errorf("Expected BaseURL %q, got %q", customEndpoint, client.config.BaseURL)
	}

	// Verify that the client actually sends requests to the override URL
