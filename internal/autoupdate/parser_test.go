...
// TestRegexParser_NoCaptureGroup verifies patterns without capture groups are rejected (B4 fix).
func TestRegexParser_NoCaptureGroup(t *testing.T) {
	_, err := NewParser("regex", `\d+\.\d+`)
	if err == nil {
		t.Fatal("expected error for pattern without capture group, got nil")
	}
...
// TestRegexParser_ValidPattern verifies valid patterns succeed at construction and extract correctly.
func TestRegexParser_ValidPattern(t *testing.T) {
	p, err := NewParser("regex", `(\d+\.\d+)`)
	if err != nil {
		t.Fatalf("expected no error for valid pattern, got %v", err)
	}
	version, err := p.Parse([]byte("version 3.14 released"))
	if err != nil {
		t.Fatalf("Parse failed: %v", err)
	}
	if version != "3.14" {
		t.Errorf("expected '3.14', got %q", version)
	}
}

// TestRegexParser_NoMatch verifies ErrNoVersionFound is returned when regex matches nothing.
func TestRegexParser_NoMatch(t *testing.T) {
	p, err := NewParser("regex", `v(\d+\.\d+)`)
	if err != nil {
		t.Fatalf("NewParser: %v", err)
	}
	_, err = p.Parse([]byte("no version here"))
	if err == nil {
