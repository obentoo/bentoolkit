...
func genHigherVersion(baseVersion string) gopter.Gen { //nolint:unused // PBT helpers for future tests
...
func genLowerVersion(baseVersion string) gopter.Gen { //nolint:unused // PBT helpers for future tests
	return gen.RegexMatch(`^[1-9][0-9]?\.[0-9]+(\.[0-9]+)?$`).SuchThat(func(v string) bool {
		return ebuild.CompareVersions(v, baseVersion) < 0
	})
}
...
// TestGenerateMessageSharedVersion tests version factoring when all packages share the same version
func TestGenerateMessageSharedVersion(t *testing.T) {
	changes := []Change{
...
// TestGenerateMessageSharedVersionUpgrade tests version factoring for upgrades
func TestGenerateMessageSharedVersionUpgrade(t *testing.T) {
	changes := []Change{
		{Type: Up, Category: "kde-frameworks", Package: "attica", Version: "6.25.0", OldVersion: "6.24.0"},
		{Type: Up, Category: "kde-frameworks", Package: "baloo", Version: "6.25.0", OldVersion: "6.24.0"},
	}

	message := GenerateMessage(changes)

	expected := "up(kde-frameworks/{attica, baloo}-6.24.0 -> 6.25.0)"
	if message != expected {
		t.Errorf("Expected %q, got %q", expected, message)
	}
}
