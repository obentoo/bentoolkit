...
func TestPackageAndVersionMatching(t *testing.T) {
	...
	properties.Property("ebuild is matched when package matches pattern AND version matches", prop.ForAll(
		func(category, pkgPrefix, pkgSuffix, version string) bool {
			...
			return len(result.Matches) == 1 && result.Matches[0].Package == pkgName
		},
		genMatcherCategoryName(),
...
func TestGlobalSearchWithAsterisk(t *testing.T) {
	...
	if len(result.Matches) != 3 {
		t.Errorf("Match() returned %d matches, want 3", len(result.Matches))
	}
...
func TestNoMatchesReturnsEmpty(t *testing.T) {
	...
	if len(result.Matches) != 0 {
		t.Errorf("Match() returned %d matches, want 0", len(result.Matches))
	}
}
...
func TestMatchPackageExactMatch(t *testing.T) {
	...
	if result.Matches[0].Package != "hello" {
		t.Errorf("Match().Package = %q, want %q", result.Matches[0].Package, "hello")
	}
}
...
// TestMatchWarningCollection tests Property 7: MatchResult Warning Collection
// **Feature: ebuild-rename, Property 7: MatchResult Warning Collection**
// **Validates: Requirements 8.1, 8.2**
//
// For any overlay with N unreadable category directories, Match() SHALL return
// exactly N warnings and still return matches from readable categories.
func TestMatchWarningCollection(t *testing.T) {
	...
	// Create a restricted-permissions category directory (chmod 000)
	restrictedCat := filepath.Join(overlayPath, "dev-libs")
	if err := os.MkdirAll(restrictedCat, 0755); err != nil {
		t.Fatalf("failed to create restricted category dir: %v", err)
	}
	if err := os.Chmod(restrictedCat, 0000); err != nil {
		t.Fatalf("failed to chmod restricted category: %v", err)
	}
	defer os.Chmod(restrictedCat, 0755) // restore so cleanup works

	matcher := NewEbuildMatcher(overlayPath)
	spec := &RenameSpec{
		Category:       "*", // Global search to hit both categories
		PackagePattern: "*",
		OldVersion:     "1.0.0",
		NewVersion:     "2.0.0",
	}
	...
	// Should have at least 1 warning for the restricted category
	if len(result.Warnings) == 0 {
		t.Error("Match() should have collected warnings for unreadable category")
	}

	// Warning should mention the category name
	foundCatWarning := false
	for _, w := range result.Warnings {
		if containsStr(w, "dev-libs") {
			foundCatWarning = true
			break
		}
	}
	if !foundCatWarning {
		t.Errorf("Warnings should contain category name 'dev-libs', got: %v", result.Warnings)
	}

	// Matches from readable categories should still be returned
	if len(result.Matches) == 0 {
		t.Error("Match() should still return matches from readable categories")
	}

	foundReadableMatch := false
	for _, m := range result.Matches {
		if m.Category == "app-misc" {
			foundReadableMatch = true
			break
		}
	}
	if !foundReadableMatch {
		t.Error("Match() should return matches from readable 'app-misc' category")
	}
}

// containsStr is a simple substring check helper for matcher tests.
func containsStr(s, substr string) bool {
	return len(s) >= len(substr) && (s == substr ||
		(len(substr) > 0 && len(s) > 0 && findSubstr(s, substr)))
}
