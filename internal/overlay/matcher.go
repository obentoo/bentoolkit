...
// Scan errors are collected as warnings in the result rather than aborting.
func (m *EbuildMatcher) Match(spec *RenameSpec) (*MatchResult, error) {
	result := &MatchResult{}
	...
	if spec.Category == "*" {
		...
		categoryMatches, categoryWarnings, err := m.matchCategory(categoryPath, spec.Category, spec)
		if err != nil {
			return nil, err
		}
		result.Matches = categoryMatches
		result.Warnings = append(result.Warnings, categoryWarnings...)
	}
...
// Returns matches and any per-package warnings encountered during scanning.
func (m *EbuildMatcher) matchCategory(categoryPath, categoryName string, spec *RenameSpec) ([]RenameMatch, []string, error) {
	...
	for _, entry := range entries {
		...
		if err != nil {
			warnings = append(warnings, fmt.Sprintf("skipping package %s: %v", pkgName, err))
			continue
		}
